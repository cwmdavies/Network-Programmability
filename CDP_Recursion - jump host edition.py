...
jumpserver_ip = "10.251.6.31"
jumpserver_username = "adm.davies"
jumpserver_password = getpass.getpass("Enter Password: ")
remote_ip = "10.145.61.10"
remote_password = jumpserver_password
...
def open_session(hostname):
    try:
        ...
        ssh = jumpssh.SSHClient()
        ssh.set_missing_host_key_policy(jumpssh.AutoAddPolicy())
        ssh.connect(hostname=hostname, port=port, username=username, password=password)
        return ssh, True
    except jumpssh.ssh_exception.AuthenticationException:
        print(
        ...
        return None, False
    except jumpssh.ssh_exception.NoValidConnectionsError:
...
def extract_cdp_neighbors(ip):
    ...
    except jumpssh.ssh_exception.SSHException:
        print(
